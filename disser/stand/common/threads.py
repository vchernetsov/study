...
class LoopStep:
    def __init__(self, frequency, sound_duration=SOUND_DURATION, sleep_time=SLEEP_TIME, ir_delay=IR_DELAY):
    ...
    def engage(self, ir: IRController, logger: Logger, missed_frequencies: list, missed_lock: threading.Lock, estimation):
        print (estimation)
        sweep_thread = self.sweep_thread(self.frequency, self.sound_duration)
        ir_thread = self.ir_thread(ir, logger, missed_frequencies, missed_lock, self.ir_delay)
        try:
            sweep_thread.join()
            ir_thread.join()
    ...
    def sweep_thread(self, frequency, sound_duration):
    ...
    def ir_thread(self, ir: IRController, logger: Logger, missed_frequencies: list, missed_lock: threading.Lock, ir_delay):
        def delayed_engage():
            time.sleep(ir_delay)
            result = ir.engage()
            if result:
...
class Loop:
    ...
    def engage(self):
        for idx, x in enumerate(self.iterable):
            ...
            try:
                x.engage(self.ir, self.logger, self.missed_frequencies, self.missed_lock, estimation)
            except FinishLoop:
                raise
            except KeyboardInterrupt:
